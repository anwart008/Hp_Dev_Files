...
export const Cart = () => {
  ...
  const reduxList = useSelector((state) => state.reduxlist);
  const reduxPrice = useSelector((state) => state.reduxprice);

  let defaultList =[];
  let defaultPrice = 0;

  if (reduxList !== undefined) {
    defaultList = reduxList
  }else{
    defaultList = [];
  }

  if (reduxPrice !== undefined) {
    defaultPrice = reduxPrice
  }else{
    defaultPrice = 0;
  }
  ...
  return (
    <div className="container-fluid row ">
      <div className="container-fluid col-sm column rounded text-center mt-3 ">
        <h2>Items in your cart</h2>
        {list.map((currElem, index) => {
          return (
            <div
              className="col-30 col-md-15 mt-5 p-3 rounded innerbox"
              key={index}
            >
              ...
              <img
                src={foodImg}
                className="img-thumbnail rounded float-right img-responsive smimg"
                alt="img not loading"
              />
            </div>
          );
      ...
      <div className="container-fluid col-sm column text-center rounded mt-3 p-3 cart">
        ...
        <div className="col-30 col-md-15 m-5 p-3 column rounded ">
          ...
          <div className="spacecbetween">
            <h4>Discount:</h4>
            <h4>₹{price - parseInt(Discount(price))}</h4>
          </div>
          <hr />
          <div className="spacecbetween">
            <h4>Total:</h4>
            <h4>₹{parseInt(Discount(price))}</h4>
          </div>
          <div>
            <Link to="/">
              <Button variant="btn btn-primary"> Back to Menu </Button>
            </Link>
          </div>
        </div>
      </div>
